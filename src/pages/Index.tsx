  ...
  return (
    <Layout>
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative container py-10 md:py-32">
          <div className="max-w-3xl animate-fade-up">
            <span className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-saffron/90 text-saffron-foreground text-xs font-semibold uppercase tracking-widest mb-4 md:mb-6">
              <Sparkles className="w-3.5 h-3.5" /> Heritage Dictionary · Lalawigan ng Quezon
            </span>
            <h1 className="font-display text-3xl md:text-7xl font-semibold text-primary-foreground text-balance leading-[1.05] mb-5">
              Ang wika natin, <em className="not-italic text-saffron">aming yaman.</em>
            </h1>
            <p className="text-sm md:text-xl text-primary-foreground/85 max-w-2xl mb-6 md:mb-10 text-balance">
      ...
      <section className="container py-8 md:py-20">
        <div className="grid md:grid-cols-5 gap-4 md:gap-8 items-center bg-card rounded-3xl border border-border shadow-soft overflow-hidden">
          <div className="md:col-span-2 relative h-40 md:h-full bg-gradient-warm">
            ...
            <div className="absolute inset-0 grid place-items-center p-8">
              <div className="text-center text-primary-foreground">
                ...
                <div className="font-display text-3xl md:text-6xl font-semibold animate-float">{wordOfDay.word}</div>
                <div className="mt-2 text-sm opacity-90">/{wordOfDay.pronunciation}/</div>
              </div>
            </div>
          </div>
          <div className="md:col-span-3 p-5 md:p-12">
            <span className="text-[10px] uppercase tracking-widest font-bold text-accent">{wordOfDay.partOfSpeech} · {wordOfDay.category}</span>
            <h2 className="font-display text-2xl md:text-3xl mt-2 mb-3">{wordOfDay.word}</h2>
      ...
      <section className="container py-6 md:py-12">
        <div className="flex items-end justify-between mb-5 md:mb-8">
          <div>
            ...
            <h2 className="font-display text-2xl md:text-4xl mt-2">Browse by District</h2>
          </div>
          <Link to="/categories" className="hidden md:block text-sm font-medium text-primary hover:underline">View all →</Link>
        </div>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {categories.map((c) => (
            <Link key={c.id} to={`/categories?c=${c.id}`} className="group bg-card rounded-2xl border border-border p-3.5 md:p-5 hover:shadow-warm hover:-translate-y-1 transition-smooth">
              <div className="text-2xl md:text-4xl mb-2 transition-smooth group-hover:scale-110">{c.icon}</div>
      ...
      <section className="container py-8 md:py-16">
        <div className="flex items-end justify-between mb-5 md:mb-8">
          <div>
            ...
            <h2 className="font-display text-2xl md:text-4xl mt-2">Words to know</h2>
          </div>
        </div>
      ...
      {/* Cultural Note */}
      <section className="container py-8 md:py-16">
        <div className="relative rounded-3xl overflow-hidden bg-primary text-primary-foreground p-6 md:p-16">
          ...
          <div className="relative max-w-3xl">
            ...
            <h2 className="font-display text-2xl md:text-4xl mb-3">Cultural Notes</h2>
            <p className="text-primary-foreground/85 leading-relaxed text-sm md:text-lg mb-5">
